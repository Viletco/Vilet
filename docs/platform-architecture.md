...
The approved production model uses a separate Supabase project. Local development remains local, Preview remains connected to the existing staging project, and `app.vilet.co` will connect only to the new production project. Privileged bootstrap commands already require an explicit environment, matching project reference, dry-run/apply intent, and exact production confirmation. The live RLS fixture runner now also requires an explicit environment and matching project reference; production fixtures additionally require exact project-reference confirmation before any temporary records are created.

The production Supabase project is `detqlxrismxlbgsgeafx`. On 2026-08-19, all three platform migrations were applied in order, the complete live RLS suite passed with temporary fixtures removed, and the canonical Vilét bootstrap created exactly one internal organization, one active owner membership, one independent platform-administrator grant, and thirteen internal entitlements. A second bootstrap made no changes, and the Phase B live verification passed every tenant-isolation and privilege-escalation scenario. Vercel Production now contains only the four runtime values required by the platform; the Supabase service-role credential remains local and is not deployed. Preview continues to use staging.

`app.vilet.co` is attached to the separate platform project through the provider-assigned CNAME and was live-verified on 2026-08-19. Vercel Authentication protects Preview only; Production serves the Vilét login publicly. DNS, Vercel domain verification, managed HTTPS, canonical production Supabase CSP targeting, HSTS, frame denial, no-store responses, `X-Robots-Tag: noindex, nofollow`, the login form, and a disallow-all `robots.txt` all passed. The `vilet.co` marketing deployment and its DNS records were not modified.
...
Production authentication and final Preview environment isolation no longer block Phase E1. On 2026-08-19, the unnecessary Preview-scoped `SUPABASE_SERVICE_ROLE_KEY` was removed and Preview was redeployed successfully at `https://vilet-platform-preview-2albdokp1-swzyfrmdarocs-projects.vercel.app`. An unauthenticated request still redirects to Vercel SSO. The protected application CSP independently proves Preview targets staging `lzohhfmfdqivnjqqwmqu`; the production CSP proves `app.vilet.co` targets production `detqlxrismxlbgsgeafx`. Neither Vercel environment contains the service-role variable, and the new Preview's browser assets contain neither the privileged credential nor the production project reference.

Phase E1 is cleared to begin on the QA branch when explicitly requested. This clearance covers only the prospect/pipeline foundation defined in the Phase E architecture; it does not authorize automated discovery, AI research, enrichment, contact storage, email, campaigns, outreach, customer rollout, a production deployment, or a merge to `main`.
